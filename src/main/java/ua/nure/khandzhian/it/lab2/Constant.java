package ua.nure.khandzhian.it.lab2;

public class Constant {

    public static final String TAG_PHONE = "Phone";
    public static final String TAG_PHONE_INDEX = "PhoneIndex";
    public static final String TAG_MODEL = "Model";
    public static final String TAG_DIAGONAL = "Diagonal";
    public static final String TAG_NUMBER_OF_CAMERA = "NumberOfCamera";
    public static final String TAG_BATTERY_CAPACITY = "BatteryCapacity";
    public static final String TAG_PRICE = "Price";
    public static final String TAG_DESCRIPTION = "Description";
    public static final String TAG_SPECIAL_FUNCTION_1 = "SpecialFunction1";
    public static final String TAG_SPECIAL_FUNCTION_2 = "SpecialFunction2";
    public static final String TAG_SPECIAL_FUNCTION_3 = "SpecialFunction3";

    public static final String ATTRIBUTE_CAPACITY = "capacity";
    public static final String ATTRIBUTE_CURRENCY = "currency";

    public static final String PHONE_XSD_NAMESPACE = "http://nure.ua/khandzhian/ITLB1 phone.xsd";
    public static final String FEATURE_TURN_VALIDATION_ON = "http://xml.org/sax/features/validation";
    public static final String FEATURE_TURN_SCHEMA_VALIDATION_ON = "http://apache.org/xml/features/validation/schema";

    public static final String PATH_TO_XML = "src/main/java/ua/nure/khandzhian/it/lab1/phone.xml";
    public static final String PATH_TO_XSD = "src/main/java/ua/nure/khandzhian/it/lab1/phone.xsd";

    public static final String DOM_XML = "src/main/java/ua/nure/khandzhian/it/lab2/dom-phone.xml";
    public static final String SAX_XML = "src/main/java/ua/nure/khandzhian/it/lab2/sax-phone.xml";
    public static final String STAX_XML = "src/main/java/ua/nure/khandzhian/it/lab2/stax-phone.xml";
    public static final String STAX2_XML = "src/main/java/ua/nure/khandzhian/it/lab2/stax2-phone.xml";
    public static final String JAXB_XML = "src/main/java/ua/nure/khandzhian/it/lab2/jaxb-phone.xml";

    public static final String PATH_TO_INVALID_XML = "src/main/java/ua/nure/khandzhian/it/lab1/invalid-phone.xml";
}
